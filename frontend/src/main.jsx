...
function HomeV6({ content, pages, setPages, articles, setArticles, admin, setAdmin, setContent }) {
  const services = [
  ...
  return <>
    ...
    <main className="reference-home">
      ...
      <ContactV2 content={content}/>
    </main>
    <footer><Logo/><p>© {new Date().getFullYear()} Laboratori Alfa.</p></footer>
    {admin && <Admin onClose={() => setAdmin(false)} content={content} setContent={setContent} articles={articles} setArticles={setArticles} pages={pages} setPages={setPages}/>}</>;
}
...
function ContactPage({ content }) { return <><HeaderV4/><main className="contact-page"><p className="catalog-index">Laboratori Alfa / Kontakt</p><h1>Jemi këtu për t’ju ndihmuar.</h1><ContactV2 content={content}/></main><footer><Logo/><p>© {new Date().getFullYear()} Laboratori Alfa.</p></footer></>; }
...
function AppV4() {
  const [content, setContent] = useState(fallback); const [pages, setPages] = useState([]); const [articles, setArticles] = useState([]); const [admin, setAdmin] = useState(false);
  ...
  return <HomeV4 content={content} pages={pages} articles={articles} setArticles={setArticles} admin={admin} setAdmin={setAdmin} setContent={setContent} setPages={setPages}/>;
}
  ...
  return <section className="contact" id="kontakt">
    <div className="contact-copy">
      ...
      <div className="contact-list">
        ...
        <p><Phone/> <span><strong>Telefoni</strong><span className="phone-links"><a href="tel:+355688546291">068 854 6291</a><a href="tel:+355682206300">068 220 6300</a></span></span></p>
      </div>
...
fallback.contactPhone = '068 854 6291\n068 220 6300';
...
function Admin({ onClose, content, setContent, articles, setArticles, pages, setPages }) {
  const blankArticle = { title: '', excerpt: '', body: '', category: 'Artikuj', imageId: null };
  const blankTopic = { slug: '', title: '', category: 'Analizat', section: '', body: '' };
  const [authenticated, setAuthenticated] = useState(false); const [password, setPassword] = useState(''); const [error, setError] = useState(''); const [notice, setNotice] = useState('');
  const [draft, setDraft] = useState(blankArticle); const [editing, setEditing] = useState(null);
  const [topics, setTopics] = useState([]); const [topicDraft, setTopicDraft] = useState(blankTopic); const [editingTopic, setEditingTopic] = useState(null);
  const loadAdminData = () => api('/api/admin/knowledge').then(setTopics);
  useEffect(() => { api('/api/admin/session').then(() => { setAuthenticated(true); return loadAdminData(); }).catch(() => {}); }, []);
  const login = async e => { e.preventDefault(); try { await api('/api/admin/login', { method: 'POST', body: JSON.stringify({ password }) }); setAuthenticated(true); setError(''); await loadAdminData(); } catch (err) { setError(err.message); } };
  const saveContent = async key => { try { await api(`/api/admin/content/${key}`, { method: 'PUT', body: JSON.stringify({ value: content[key] }) }); setNotice('Ndryshimi u ruajt.'); } catch (err) { setNotice(err.message); } };
  ...
  const uploadHero = async file => { try { const form = new FormData(); form.append('image', file); const response = await fetch('/api/admin/images', { method: 'POST', body: form }); if (!response.ok) throw new Error('Ngarkimi dështoi.'); const image = await response.json(); const value = image.url; setContent(old => ({ ...old, heroImage: value })); await api('/api/admin/content/heroImage', { method: 'PUT', body: JSON.stringify({ value }) }); setNotice('Fotoja kryesore u ndryshua.'); } catch (err) { setNotice(err.message || 'Ngarkimi dështoi.'); } };
  const saveArticle = async e => { e.preventDefault(); try { const item = editing ? await api(`/api/admin/articles/${editing}`, { method: 'PUT', body: JSON.stringify(draft) }) : await api('/api/admin/articles', { method: 'POST', body: JSON.stringify(draft) }); setArticles(old => editing ? old.map(article => article.id === item.id ? item : article) : [item, ...old]); setEditing(null); setDraft(blankArticle); setNotice('Artikulli u ruajt.'); } catch (err) { setNotice(err.message); } };
  const saveTopic = async e => { e.preventDefault(); try { const item = editingTopic ? await api(`/api/admin/knowledge/${encodeURIComponent(editingTopic)}`, { method: 'PUT', body: JSON.stringify(topicDraft) }) : await api('/api/admin/knowledge', { method: 'POST', body: JSON.stringify(topicDraft) }); const next = editingTopic ? topics.map(topic => topic.slug === item.slug ? item : topic) : [...topics, item]; setTopics(next); setPages(next.map(({ slug, title, category, section }) => ({ slug, title, category, section }))); setEditingTopic(null); setTopicDraft(blankTopic); setNotice('Tema e katalogut u ruajt.'); } catch (err) { setNotice(err.message); } };
  const removeTopic = async slug => { if (!window.confirm('Ta fshijmë këtë temë?')) return; try { await api(`/api/admin/knowledge/${encodeURIComponent(slug)}`, { method: 'DELETE' }); const next = topics.filter(topic => topic.slug !== slug); setTopics(next); setPages(next.map(({ slug: pageSlug, title, category, section }) => ({ slug: pageSlug, title, category, section }))); setNotice('Tema u fshi.'); } catch (err) { setNotice(err.message); } };
  ...
  return <div className="admin-overlay"><aside className="admin-panel"><div className="admin-top"><div><p className="eyebrow">Panel privat</p><h2>Menaxho faqen</h2></div><button className="close" onClick={onClose}><X/></button></div>{notice && <p className="admin-notice" role="status">{notice}</p>}
    <section><h3>Kontakt dhe tekste</h3>{[['about','Përshkrimi i laboratorit'],['history','Historia'],['mission','Misioni'],['contactAddress','Adresa'],['contactHours','Orari'],['contactPhone','Telefoni']].map(([key, label]) => <label className="edit-field" key={key}><span>{label}</span><textarea value={content[key] || ''} onChange={e => setContent(old => ({ ...old, [key]: e.target.value }))} onBlur={() => saveContent(key)}/></label>)}<label className="upload"><Upload size={16}/> Ndrysho foton kryesore të cover-it<input type="file" accept="image/*" onChange={e => e.target.files[0] && uploadHero(e.target.files[0])}/></label></section>
    <section><div className="admin-title"><h3>{editing ? 'Redakto artikullin' : 'Artikull i ri'}</h3>{editing && <button onClick={() => { setEditing(null); setDraft(blankArticle); }}>Anulo</button>}</div><form className="article-form" onSubmit={saveArticle}><input required placeholder="Titulli" value={draft.title} onChange={e => setDraft(item => ({ ...item, title: e.target.value }))}/><input required placeholder="Kategoria" value={draft.category} onChange={e => setDraft(item => ({ ...item, category: e.target.value }))}/><textarea required placeholder="Përmbledhja" value={draft.excerpt} onChange={e => setDraft(item => ({ ...item, excerpt: e.target.value }))}/><textarea placeholder="Teksti i plotë" value={draft.body || ''} onChange={e => setDraft(item => ({ ...item, body: e.target.value }))}/><label className="upload"><Upload size={16}/>{draft.imageId ? 'Foto e bashkëngjitur' : 'Shto foto (maks. 5 MB)'}<input type="file" accept="image/*" onChange={e => e.target.files[0] && upload(e.target.files[0])}/></label><button className="button primary">Ruaj artikullin <Check size={16}/></button></form><h3 className="admin-subheading">Artikujt ekzistues</h3>{articles.map(article => <div className="admin-article" key={article.id}><span>{article.title}</span><button onClick={() => { setEditing(article.id); setDraft(article); }}>Redakto</button><button className="danger" onClick={async () => { await api(`/api/admin/articles/${article.id}`, { method: 'DELETE' }); setArticles(old => old.filter(item => item.id !== article.id)); }}>Fshi</button></div>)}</section>
    <section><div className="admin-title"><h3>{editingTopic ? 'Redakto temën / analizën' : 'Temë ose analizë e re'}</h3>{editingTopic && <button onClick={() => { setEditingTopic(null); setTopicDraft(blankTopic); }}>Anulo</button>}</div><p className="admin-hint">Kategoria dhe sektori përcaktojnë vendin e temës në katalog.</p><form className="article-form" onSubmit={saveTopic}><input placeholder="Adresa e temës (opsionale, p.sh. ferritina)" disabled={!!editingTopic} value={topicDraft.slug} onChange={e => setTopicDraft(item => ({ ...item, slug: e.target.value }))}/><input required placeholder="Titulli i analizës ose temës" value={topicDraft.title} onChange={e => setTopicDraft(item => ({ ...item, title: e.target.value }))}/><input required placeholder="Kategoria (p.sh. Analizat)" value={topicDraft.category} onChange={e => setTopicDraft(item => ({ ...item, category: e.target.value }))}/><input required placeholder="Sektori (p.sh. Hormonet)" value={topicDraft.section} onChange={e => setTopicDraft(item => ({ ...item, section: e.target.value }))}/><textarea placeholder="Përmbajtja dhe udhëzimet" value={topicDraft.body || ''} onChange={e => setTopicDraft(item => ({ ...item, body: e.target.value }))}/><button className="button primary">Ruaj temën <Check size={16}/></button></form><h3 className="admin-subheading">Temat në katalog</h3>{topics.map(topic => <div className="admin-article" key={topic.slug}><span><strong>{topic.title}</strong><small>{topic.category} · {topic.section}</small></span><button onClick={() => { setEditingTopic(topic.slug); setTopicDraft(topic); }}>Redakto</button><button className="danger" onClick={() => removeTopic(topic.slug)}>Fshi</button></div>)}</section>
  </aside></div>;
}
